...
const int BsplineOptimizer::YAWING = (1 << 10);
...
void BsplineOptimizer::setParam(ros::NodeHandle& nh) {
  ...
  nh.param("optimization/max_dyaw", max_dyaw_, -1.0);
  nh.param("optimization/max_ddyaw", max_ddyaw_, -1.0);
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  if (cost_function_ & YAWING) cost_str += " yaw  |";
...
void BsplineOptimizer::optimize() {
  ...
  if(bmin.size()== 0 || bmax.size() ==0){
    ROS_ERROR("Error in this optimize in bmin and bmax");
  }
  ...
  if(q.size() == 0){
    ROS_ERROR("Error in this optimize in q.size()");
  }
  try {
    double final_cost;
...
void BsplineOptimizer::calcYawFeasibilityCost(const vector<Eigen::Vector3d>& q, const double& dt,
  ...
  for (int i = 0; i < q.size() - 2; ++i) {
    Eigen::Vector3d ai = (q[i + 2] - 2 * q[i + 1] + q[i]) * dt_inv2;
    for (int k = 0; k < 3; ++k) {
      double ad = fabs(ai[k]) - max_ddyaw_;
      if (ad > 0.0) {
        cost += pow(ad, 2);
        double sign = ai[k] > 0 ? 1.0 : -1.0;
        double tmp = 2 * ad * sign * dt_inv2;
...
void BsplineOptimizer::combineCost(
  ...
  if (cost_function_ & YAWING) {
    double f_feasibility = 0.0, gt_feasibility = 0.0;
    calcYawFeasibilityCost(g_q_, dt, f_feasibility, g_yaw_, gt_feasibility);
    f_combine += 1 * f_feasibility;
    for (int i = 0; i < point_num_; i++)
      for (int j = 0; j < dim_; j++)
        grad[dim_ * i + j] += 1 * g_yaw_[i](j);
    if (optimize_time_) grad[variable_num_ - 1] += 1 * gt_feasibility;
  }
