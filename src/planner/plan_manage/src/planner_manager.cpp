...
#include <plan_manage/am_traj.hpp>
...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d& start_pt,
    ...
    const Eigen::Vector3d& end_pt, const Eigen::Vector3d& end_vel, double& first_time, const double& time_lb) {
  ...
  bool use_am_traj = false;
  first_duration = 0;
  if(use_am_traj)
  {

    cout << "enter am_traj..." << endl;
    auto am_start = ros::Time::now();
    AmTraj amTrajOpt(1024, 16, 0.4,
                     2, 2, 23, 0.02);
    vector<Eigen::Vector3d> pathList;
    pathList.push_back(start_pt);
    // pathList.push_back(middle_pt);
    pathList.push_back(end_pt);
    Trajectory traj_am = amTrajOpt.genOptimalTrajDTC(pathList, start_vel, start_acc, end_vel, end_vel);
    vector<Eigen::Vector3d> am_point_set, am_start_end_derivatives;
    double ts = pp_.ctrl_pt_dist / pp_.max_vel_;
    vector<double> time_duration_list = traj_am.getDurations();
    int seg_num = floor(time_duration_list[0] / ts);
    double ts_ = time_duration_list[0] / (seg_num + 1);

    cout << "am_traj success..." << endl;
    first_time = time_duration_list[0];
    first_duration = time_duration_list[0];
    if (traj_am.getPieceNum() > 0)
    {
      for (double t = ts_; t < time_duration_list[0] + 0.01; t += ts_)
      {
        Eigen::Vector3d curPos = traj_am.getPos(t);
        am_point_set.push_back(curPos);
      }
    }

    am_start_end_derivatives.push_back(start_vel);
    am_start_end_derivatives.push_back(end_vel);
    am_start_end_derivatives.push_back(start_acc);
    am_start_end_derivatives.push_back(end_vel);

    Eigen::MatrixXd ctrl_pts;
    NonUniformBspline::parameterizeToBspline(
        ts_, am_point_set, am_start_end_derivatives, pp_.bspline_degree_, ctrl_pts);
    NonUniformBspline init(ctrl_pts, pp_.bspline_degree_, ts_);

    if (first_duration < time_lb)
    {
      ...
      bspline_optimizers_[0]->optimize(ctrl_pts, ts_, cost_function, 1, 1);
      local_data_.position_traj_.setUniformBspline(ctrl_pts, pp_.bspline_degree_, ts_);
    }
    // local_data_.position_traj_ = init;
    updateTrajInfo();
    double am_plan_time = (ros::Time::now().toSec() - am_start.toSec());
    cout << "<<******************am_plan_time******************>>: " << am_plan_time << endl;
    return true;
  }
  ...
  if (use_guide)
  {
    ...
    kino_path_finder_->guide_path = guide_path;
    if (guide_path.size() == 2)
      kino_path_finder_->use_guide_path = false;
    else
      kino_path_finder_->use_guide_path = true;
  }
  else
    kino_path_finder_->use_guide_path = false;
  ...
  bspline_optimizers_[0]->ld_end_ = 0.5;
...
//   const Eigen::Vector3d &start_yaw, const double &end_yaw, bool lookfwd, const double &relax_time, const double &time_ratio)
// {

// const int seg_num = 12;

// // double dt_yaw = local_data_.duration_ / seg_num; // time of B-spline segment
// double dt_yaw = time_ratio / seg_num;
// Eigen::Vector3d start_yaw3d;
// if (fabs(start_yaw[2]) < 20)
//   start_yaw3d = start_yaw;
// else start_yaw3d = {start_yaw[0], start_yaw[1], 0};
...
//   //   double tf = min(time_ratio, tc + forward_t);
...
// cout << "debug rapid change of yaw" << endl;
// cout << fabs(start_yaw3d[0] - end_yaw3d[0]) << endl;
// cout << start_yaw3d[0] << end_yaw3d[0] << endl;
...
// int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::START | BsplineOptimizer::END | BsplineOptimizer::YAWING;
...
// bspline_optimizers_[1]->ld_end_ = 10;
...
void FastPlannerManager::updateTrajInfo() {
  ...
  local_data_.first_duration = local_data_.duration_;
...
void FastPlannerManager::planYawExplore(const Eigen::Vector3d& start_yaw, const double& end_yaw,
    bool lookfwd, const double& relax_time, const double& time_ratio) {
  const int seg_num = 12;
...
void FastPlannerManager::planYYawExplore(
  const Eigen::Vector3d &start_yaw, const double &end_yaw, bool lookfwd, const double &relax_time, const double &time_ratio)
{
...
vector<Eigen::Vector3d> start = {Eigen::Vector3d(start_yaw3d[0], 0, 0),
                                 ...
                                 Eigen::Vector3d(start_yaw3d[2], 0, 0)};
vector<Eigen::Vector3d> end = {Eigen::Vector3d(end_yaw3d[0], 0, 0), Eigen::Vector3d(0, 0, 0)};
bspline_optimizers_[2]->ld_end_ = 10;
bspline_optimizers_[2]->setBoundaryStates(start, end);
bspline_optimizers_[2]->setWaypoints(waypts, waypt_idx);
bspline_optimizers_[2]->optimize(yaw, dt_yaw, cost_func, 1, 1);

// std::cout << "2: " << (ros::Time::now() - t1).toSec() << std::endl;

// Update traj info
local_data_.yaw1_traj_.setUniformBspline(yaw, 3, dt_yaw);
local_data_.yawdot1_traj_ = local_data_.yaw_traj_.getDerivative();
local_data_.yawdotdot1_traj_ = local_data_.yawdot_traj_.getDerivative();
plan_data_.dt_yaw_ = dt_yaw;
...
void FastPlannerManager::planYawForYawing(const Eigen::Vector3d &start_yaw, const double &end_yaw, bool lookfwd, const double & relax_time, const double& time_ratio){
  ...
  // }
    ...
    // }
    // std::cout << "" << std::endl;
    ROS_ERROR("It will be crashed in position 2");
    auto t1 = ros::Time::now();
    ...
    vector<Eigen::Vector3d> end = {Eigen::Vector3d(end_yaw3d[0], 0, 0), Eigen::Vector3d(0, 0, 0)};
    bspline_optimizers_[1]->ld_end_ = 10;
    //ROS_ERROR("It will be crush! position 3");
    bspline_optimizers_[1]->setBoundaryStates(start, end);
    // std::cout << "dt_yaw: " << dt_yaw << ", cost_func: " << cost_func << std::endl;
    // bspline_optimizers_[2]->setWaypoints(waypts, waypt_idx);
    bspline_optimizers_[1]->optimize(yaw, dt_yaw, cost_func, 1, 1);
    ROS_ERROR("It will be crush! position 5");
