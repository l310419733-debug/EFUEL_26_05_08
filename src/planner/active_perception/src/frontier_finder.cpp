...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, ros::NodeHandle& nh) {
  ...
  nh.param("frontier/x_bound_min", x_bound_min, -1.0);
...
void FrontierFinder::computeFrontiersToVisit(const Vector3d& cur_pos) {
  first_new_ftr_ = frontiers_.end();
  int new_num = 0;
  int new_dormant_num = 0;
  const double VISBLE_SIMILAR_THRESH = 0.1;
  // Try find viewpoints for each cluster and categorize them according to viewpoint number
  for (auto& tmp_ftr : tmp_frontiers_) {
    // Search viewpoints around frontier
    sampleViewpoints(tmp_ftr);
    ...
    [&](const Viewpoint& v1, const Viewpoint& v2){
      if(v1.visib_num_ != v2.visib_num_){
        double visble_ratio = fabs(v1.visib_num_ - v2.visib_num_) /
                              static_cast<double>(max(v1.visib_num_, v2.visib_num_));
        if(visble_ratio < VISBLE_SIMILAR_THRESH){
          double dist1 = (v1.pos_ - cur_pos).norm();
          double dist2 = (v2.pos_ - cur_pos).norm();
          return dist1 < dist2;
        }
...
//     vector<vector<double>>& yaws, vector<bool>& small_area) {
//   points.clear();
//   yaws.clear();
...
//           double dist = (view.pos_ - cur_pos).norm();
//           if (dist < min_candidate_dist_) continue;
...
//             Vector3d dir = view.pos_ - cur_pos;
//             target_yaw = atan2(dir.y(), dir.x());
//           }else {

//             target_yaw = view.yaw_;
//           }
//           pts.push_back(view.pos_);
//           ys.push_back(target_yaw);
//         }

//         if(pts.size() < view_num){
//           for(auto view : frontier.viewpoints_){
//             if(pts.size() >= view_num)break;
//             if(view.visib_num_ <= visib_thresh)continue;
            
//             double dist = (view.pos_ - cur_pos).norm();
//             if(dist < min_candidate_dist_)continue;
            
//             double target_yaw = (dist > dist_threshold)?
//                 atan2(view.pos_.y() - cur_pos.y(), view.pos_.x() - cur_pos.x()) : view.yaw_;
//             pts.push_back(view.pos_);
//             ys.push_back(target_yaw);
//           }
//         }
...
//         //   for (auto view : frontier.viewpoints_) {
//         //     // if (pts.size() >= view_num || view.visib_num_ <= visib_thresh) break; 6.17修改
//         //     if(pts.size() >= view_num)break;
//         //     if((view.pos_ - cur_pos).norm() < min_candidate_dist_)continue;
//         //     pts.push_back(view.pos_);
//         //     ys.push_back(view.yaw_);
...
// const double dist_threshold = 1.0;
  ...
  for (auto frontier : frontiers_) {
    if (frontier.id_ == id){
            ...
            points.push_back(pts);
            yaws.push_back(ys);
            small_area.push_back(frontier.smallArea);
      }
    }
  }
}
...
void FrontierFinder::getFullCostMatrix(const Vector3d& cur_pos, const Vector3d& cur_vel,
  ...
  // std::cout << "" << std::endl;
    ...
    mat.leftCols<1>().setZero();
    std::cout << "find error:" << frontiers_.size() << std::endl;
    ...
    for (auto &ftr : frontiers_)
      {
        ...
        double ftr_x = ftr.average_[0];
        double ftr_y = ftr.average_[1];
        double min_x = std::min(x_bound_max - ftr_x, ftr_x - x_bound_min);
        double min_y = std::min(y_bound_max - ftr_y, ftr_y - y_bound_min);

        double min_dis_edge = std::min(min_x, min_y);
        // if(min_dis_edge > x_bound/3 || min_dis_edge > y_bound/3 ) min_dis_edge = 0;
        if ((min_x < 4 || min_y < 4) && (cur_pos - vj.pos_).norm() < 3)
        {
          if (min_dis_edge == min_x)
            min_dis_edge -= ((y_bound_max - y_bound_min) / 2 - min_y) * 3.0;
          else
            min_dis_edge -= ((x_bound_max - x_bound_min) / 2 - min_x) * 3.0;
        }
        ...
        // if(std::min(ftr_x + x_bound/2, x_bound/2 - ftr_x) < x_bound/10 && std::min(y_bound/2 - ftr_y, ftr_y + y_bound/2) < y_bound/10)
        //   min_dis_edge *= 0.5;
        // cout<<"min_dis_edge: "<<min_dis_edge<<endl;
        // if(min_dis_edge > 5) min_dis_edge = 0;

        Vector3d pos_dir = (cur_pos - vj.pos_).normalized();
        double angle = fabs(acos(pos_dir.dot(cur_vel.normalized())));
        if (angle > M_PI)
          angle = 2 * M_PI - angle;

        // cout<<"angle: "<<angle<<endl;

        Eigen::Vector3d ftr_dir = (ftr.average_ - vj.pos_).normalized();

        double pre_info = 5;

        if ((cur_pos - vj.pos_).norm() > 3 && (cur_pos - vj.pos_).norm() < 8 && min_dis_edge > 5)
          min_dis_edge *= 1.3;
        
        if ((cur_pos - vj.pos_).norm() > 8)
          min_dis_edge *= 1.5;

        if(vj.pos_[2] > 2.5  && vj.pos_[2] <= 4)
          min_dis_edge += 10;
        if(vj.pos_[2] > 4)
          min_dis_edge += 20;
        ...
        // if ((cur_pos - vj.pos_).norm() > 5.0)
        //   min_dis_edge *= 1.5;
        // if(vj.pos_[2] > 2.5)
        //   min_dis_edge *= 1.5 + vj.pos_[2] - 2.5;

        double pre_para = 5;
        if ((cur_pos - vj.pos_).norm() < 3 && ftr.average_[2] < 2)
        {
          for (int i = 1; i < 30; i++)
          {
            int temp_num = 0;
            Eigen::Vector3d temp_pos = ftr.average_ + ftr_dir * 0.1 * i;
            if (!edt_env_->sdf_map_->isInBox(temp_pos) || edt_env_->sdf_map_->getInflateOccupancy(temp_pos) == 1)
            {
              cout << "small area higher priority" << endl;
              ftr.smallArea = true;
              if (i > 5)
              {
                pre_info = 0.1 * i;
            ...
            if (i > 5 && edt_env_->sdf_map_->getOccupancy(temp_pos) == SDFMap::FREE)
            {
              ftr.smallArea = true;
              if (i > 15 && i < 30)
                pre_info = 3.5;
        ...
        // select frontier containing surface unknown voxel

        // cout<<"test "<<endl;
        double flyCost = ViewNode::computeCost(cur_pos, vj.pos_, cur_yaw[0], vj.yaw_, cur_vel, cur_yaw[1], path) + min_dis_edge * 0.5 + angle * 0.05 - (5 - pre_info) * pre_para;

        mat(0, j++) = flyCost;

        // cout<<"====================: "<<test<<";"<<surfaceVoxelNum<<endl;
      }
      // std::cout << "" << std::endl;
    }
...
inline bool FrontierFinder::isNeighborOccupied(const Eigen::Vector3i &voxel)
{
  ...
  for (auto nbr : nbrs)
  {
    if ((edt_env_->sdf_map_->getOccupancy(nbr) == SDFMap::OCCUPIED || !edt_env_->sdf_map_->isInMap(nbr)) && nbr[2] > 10)
    {
      // cout<<"zAxis: "<<nbr[2]<<endl;
      return true;
    }
  }
  return false;
