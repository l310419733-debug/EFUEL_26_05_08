  ...
  for (int x = 0; x < grid_num_[0]; ++x) {
    for (int y = 0; y < grid_num_[1]; ++y) {
      for (int z = 0; z < grid_num_[2]; ++z) {
        ...
        grid.explored_by_ = -1;
        grid.is_passable_ = false;
        grid.last_accessed_time_ = 0.0;
        if (level_ == 1)
          grid.active_ = true;
        else
          grid.active_ = false;
...
//     for(const GridCell &grid_cell : uniform_grid){
//       dim += grid_cell.centers_free_active_.size();
//       dim += grid_cell.centers_unknow_active_.size();

//       int cell_id = grid_cell.id_;
      
//       for(int i = 0; i < (int)grid_cell.centers_freez_active_.size();i++){
...
void UniformGrid::updateGridData(const int& drone_id, vector<int>& grid_ids, vector<int>& parti_ids,
  ...
  for (int i = 0; i < grid_data_.size(); ++i) {
    ...
    if (grid.need_divide_) {
      ...
      grid.active_ = false;
      grid.explored_by_ = drone_id;//4.22修改
    }
  ...
  if (!initialized_) {
    ...
    for (auto it = grid_ids.begin(); it != grid_ids.end();) {
       //4.22修改 修改网格分配逻辑，跳过已被其他无人机探索的非活跃网络
...
//4.10 修改 增加超时激活机制（20秒未访问视为可通行）
  // double current_time = ros::Time::now().toSec();
  // for(auto& grid: grid_data_){
  //   if(!grid.active_ && grid.is_passable_ &&
  //     (current_time - grid.last_accessed_time_ > 20.0)){
  //       grid.active_ = true;
  //       // grid.last_accessed_time_  = current_time;
  //       // ROS_WARN("Reactivate grid %d due to timeout", grid.id_);
  //     }
  // }

  Eigen::Vector3i id;
  const int drone_num = positions.size();
  for(int i; i < drone_num; i++){
    posToIndex(positions[i], id);
    int adr = toAddress(id);
    auto& grid = grid_data_[adr];
    if (!insideGrid(id))continue;
    else {
        if(tn - grid.last_access_time >= MIN_INTERVAL){
          if(have_overlap(grid.vmin_, grid.vmax_, update_min, update_max) && grid.expl_comple_){
            grid.access_times_ += 1;
        }
        }
      }
      grid.last_access_time = tn;
  }

  grid_ids.insert(grid_ids.end(), rediscovered_ids.begin(), rediscovered_ids.end());
}

// void UniformGrid::activateGridForShort(const Vector3d& start, const Vector3d& goal){
//   vector<Vector3d> samples = Astar::generateSamplingPoints(start, goal, 0.5); // 0.5米间隔采样

//   // 检查采样点所在网格
//   for (const auto& pt : samples) {
//     int grid_id = posToGridId(pt);
//     if (grid_id < 0 || grid_id >= grid_data_.size()) continue;

//     auto& grid = grid_data_[grid_id];
//     // 激活条件：非活跃、可通行、未被其他无人机独占
//     if (!grid.active_ && grid.is_passable_ && grid.explored_by_ == -1) {
//       grid.active_ = true;         // 动态激活
//       grid.last_accessed_time_ = ros::Time::now().toSec();
//       ROS_WARN("Activate grid %d for shortcut", grid_id);
//     }
//   }
// }
...
void UniformGrid::updateGridInfo(const Eigen::Vector3i& id) {
  ...
  if (level_ == 1 && grid.active_ && free > min_free_ && !grid.access_times_) {
    grid.need_divide_ = true;
  }

  if(free <= min_free_ || !grid.access_times_){
    grid.expl_comple_ = true;
  }
}
...
bool UniformGrid::isRelevant(const GridInfo& grid) {
  ...
  if(level_ == 1){
    return (grid.unknown_num_ >= min_unknown_ || !grid.contained_frontier_ids_.empty()) && !grid.access_times_;
  }
  else if(level_ > 1){
    return grid.unknown_num_ >= min_unknown_ || !grid.contained_frontier_ids_.empty();
  }
...
void UniformGrid::activateGridForShort(const vector<Eigen::Vector3d>& path){
