...
public:
  ...
  ViewNode(const Vector3d& p, const double& y, const bool& smallFrontier);
  ViewNode() {
  }
  ~ViewNode() {
  }
  ...
  static double computeCostfrontier(const Vector3d& p1, const Vector3d& p2, const double& y1,
    const double& y2, const Vector3d& v1, const double& yd1, vector<Vector3d>& path);
  // Data
  ...
  double g_forward_;
  double h_forward_;
  shared_ptr<ViewNode> parent_forward_;
  bool smallArea;

  double g_backward_;
  double h_backward_;
  shared_ptr<ViewNode> parent_backward_;
