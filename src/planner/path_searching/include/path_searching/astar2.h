...
public:
  ...
  enum { REACH_HORIZON = 0, REACH_END = 1, NO_PATH = 2 };

  void init(ros::NodeHandle& nh, const EDTEnvironment::Ptr& env);
  void reset();
  int search(
      const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt, bool local_use);
  ...
  // bool checkPathLength(const std::vector<Eigen::Vector3d>& start, const std::vector<Eigen::Vector3d>& goal, std::vector<Eigen::Vector3d>& path);
  // void activateGridForShortcut(const Vector3d& start, const Vector3d& goal);
  // std::vector<Eigen::Vector3d> generateSamplingPoints(const Eigen::Vector3d& start, const Eigen::Vector3d& goal, double interval);
  // double computerPathCost(const Vector3d &p1, const Vector3d &p2, const double &y1,
  //   const double &y2, const Vector3d &v1, const double &yd1,
  //   const vector<Vector3d> &path, const double &len, bool verbose);
  // double searchPathUnknown(const Vector3d &p1, const Vector3d &p2, vector<Vector3d> &path)
  ...
  bool judgePointsCollision(const Eigen::Vector3d& first_point, const Eigen::Vector3d& sec_point);
