...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt, bool local_use) {
      ...
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          if(local_use)
          {
            ...
            if(edt_env_->sdf_map_->getInflateOccupancy(temp_index)==1 ||edt_env_->sdf_map_->getOccupancy(temp_index)){
              safe = false;
          }
          }
...
// bool Astar::checkPathLength(const std::vector<Eigen::Vector3d>& start, const std::vector<Eigen::Vector3d>& goal, std::vector<Eigen::Vector3d>& path)
// {
//   double direct_dist = (goal - start).norm();
//   double path_dist = pathLength(path);
//   if(path_dist > 1.5*direct_dist){
//     // UniforGrid::activateGridForShortcut(start, goal);
//     return false; 
//   }
//   return true;
// }
