...
// #include <exploration_manager/Track.h>
...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  fd_->state_str_ = { "INIT", "WAIT_TRIGGER", "YAWING", "PLAN_TRAJ", "PUB_TRAJ", "EXEC_TRAJ", "FINISH", "IDL"
                                                                                              "E" };
...
//3.18
  drone_state_est_pub_ = nh.advertise<geometry_msgs::PoseArray>("/measurement/pose_array", 10);
  drone_state_est_sub_ = nh.subscribe(
    "/kf/good_tracks_pose_array", 10, &FastExplorationFSM::droneKfEstimatePoseCallback, this);
  drone_est_path_pub_ =  nh.advertise<nav_msgs::Path>("/kalmanfilter/path", 10);
  // ros::Subscriber cloud_sub = nh.subscribe("/sdf_map/occupancy_local", 10, cloudCallback);
  // drone_est_track_pub_ =  nh.advertise<nav_msgs::Path>("/kalmanfilter/track", 10);
  ...
  pg_T_vio_sub_ = nh.subscribe("loop_fusion/pg_T_vio", 10,&FastExplorationFSM::pgTVioCallback, this);
...
void FastExplorationFSM::cmdCallback(){
  if(!receive_traj_)
    return;
  // ROS_ERROR("has been enter cmdCallback!");
  mtx_plan_.lock();
  ...
  // state* last_plan_node = NULL;
  if(plan_.size() > 1){
      plan_.pop_front();
      // plan_.pop_back();
      //next_goal = plan_.back();
      //plan_.clear();
    // plan_.clear();
      ROS_ERROR("plan_ size : %d", plan_.size());
  }
  else
  {
    ...
    // ROS_ERROR("entry cmdCallback, but no state no change");
    // flightEnd = true;
    if (state_ == YAWING)
    {
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case YAWING:
    {
      ...
      if(yawing == false)
      {
        Eigen::Vector3d init_yaw(0, 0, 0);
        init_yaw << fd_->odom_yaw_, 0, 0;
        expl_manager_->yawingBeforeFlight(init_yaw);
        mtx_plan_.lock();
        state init_state;
        Eigen::Vector3d init_pt = fd_->odom_pos_;
        ...
        ROS_ERROR("newPlanList size: %d", newPlanList.size());
        // if(addPath){
        //   ROS_ERROR("has been added!");
        // }
      }
      yawing = true;
      break;
...
int FastExplorationFSM::callExplorationPlanner() {
  ...
  ros::Time time_r = current_time + ros::Duration(fp_->replan_time_);
  ...
  if (plan_.empty()) {
    ROS_ERROR("plan_ is empty, cannot get start state!");
    return FAIL;
  }
  int index = plan_.size() - 1 - k_end_whole;
  if (index < 0 || index >= plan_.size()) {
  ROS_ERROR("Invalid index %d for plan_ (size: %zu)", index, plan_.size());
  return FAIL;
  }
  A = plan_[index];
  ...
  //       fd_->start_pt_, fd_->start_pt_, fd_->start_acc_, fd_->start_yaw_);
  // }
  ...
  if (res == SUCCEED) {
    ...
    ROS_INFO("Plan succeed, start time: %f", time_r.toSec());
    auto info = &planner_manager_->local_data_;
    ...
    Eigen::VectorXd pos_knots = info->position_traj_.getKnot();
    double pos_t_min = pos_knots(0);
    double pos_t_max = pos_knots(pos_knots.size() - 1);

    double t_start = std::max(0.01, pos_t_min);
    double t_end = std::min(pos_t_max - 1e-2, pos_t_max);
    if(t_start > t_end){
      ROS_ERROR("t_start: %f, t_end: %f", t_start, t_end);
      return FAIL;
    }
    // double traj_duration = info->duration_;
    // ROS_ERROR("traj_duration: %f", traj_duration);
    
    for(double i = t_start; i <= t_end; i += 0.01) {
      if(i < pos_t_min || i > pos_t_max){
        ROS_ERROR("i: %f, pos_t_min: %f, pos_t_max: %f", i, pos_t_min, pos_t_max);
        return FAIL;
      }
      state temp_state;
      temp_state.pos = info->position_traj_.evaluateDeBoorT(i);
      ROS_INFO("error no here in 1");
      ...
      if(expl_manager_->manyViewPoints && i >= expl_manager_->first_time_duration){
        ...
        Eigen::VectorXd yaw1_knots = info->yaw1_traj_.getKnot();
        double yaw1_t_min = yaw1_knots(0);
        double yaw1_t_max = yaw1_knots(yaw1_knots.size() - 1);
        if(temp_t < yaw1_t_min || temp_t > yaw1_t_max){
          ROS_ERROR("temp_t: %f, yaw1_t_min: %f, yaw1_t_max: %f", temp_t, yaw1_t_min, yaw1_t_max);
          return FAIL;
        }
...
void FastExplorationFSM::visualize(int content) {
  ...
  if (content == 1) {
    ...
    // auto grid_tour1 = expl_manager_->ed_->grid_tour2_;
...
void FastExplorationFSM::frontierCallback(const ros::TimerEvent& e) {
  ...
  if (state_ == WAIT_TRIGGER || state_ == YAWING || state_ == FINISH) {
    ...
    bool status = expl_manager_->findGlobalTourOfGrid(
        { fd_->odom_pos_ }, { fd_->odom_vel_ }, tmp_id1, tmp_id2, { expl_manager_->ed_->next_yaw_ }, { fd_->start_yaw_ }, true);
...
void FastExplorationFSM::droneKfEstimatePoseCallback(const geometry_msgs::PoseArray::ConstPtr& msg)
{
  const int max_path_length = 5000; 
  
  // nav_msgs::Path tracks_msgs_;
 

  path_msgs_.header.frame_id = "world";
  ...
  if (path_msgs_.poses.size() > max_path_length) {
    path_msgs_.poses.erase(path_msgs_.poses.begin(), path_msgs_.poses.end() - max_path_length);
  }

  
  drone_est_path_pub_.publish(path_msgs_);
  // tracks_msgs_.poses.push_back(drone_est_path_pub_.publish(path_msgs_);.poses.back());
  // drone_est_track_pub_.publish(tracks_msgs_);
}
...
void FastExplorationFSM::droneStateTimerCallback(const ros::TimerEvent& e) {
  ...
  geometry_msgs::Pose pose_est1;
  msg.pos = { float(state.pos_[0]), float(state.pos_[1]), float(state.pos_[2]) };
  // pose_est1.position.x = float(state.pos_[0]);
  // pose_est1.position.y = float(state.pos_[1]);
  // pose_est1.position.z = float(state.pos_[2]);
  // pose_array.poses.push_back(pose_est1);
...
void FastExplorationFSM::droneStateMsgCallback(const exploration_manager::DroneStateConstPtr& msg) {
  ...
  // pose_est1.position.x = msg->pos[0];
  // pose_est1.position.y = msg->pos[1];
  // pose_est1.position.z = msg->pos[2];
  ...
  // pose_est1.orientation.x = msg->yaw;
...
void FastExplorationFSM::optTimerCallback(const ros::TimerEvent& e) {
  ...
  expl_manager_->allocateGrids(positions, velocities, { first_ids1, first_ids2 },
      { second_ids1, second_ids2 }, opt_ids, ego_ids, other_ids,{ expl_manager_->ed_->next_yaw_ },{ fd_->start_yaw_ });
  ...
  // Check results
  //fd_->start_yaw_, expl_manager_->ed_->next_yaw_
  double prev_app1 = expl_manager_->computeGridPathCost(state1.pos_, state1.grid_ids_, first_ids1,
      { first_ids1, first_ids2 }, { second_ids1, second_ids2 }, velocities, true, expl_manager_->ed_->next_yaw_, fd_->start_yaw_);
...
bool FastExplorationFSM::newPathCallback(vector<state> path, int start_index)
{
  ...
  receive_traj_ = true;
  ROS_ERROR("plan_ has points: %d.", plan_.size());
