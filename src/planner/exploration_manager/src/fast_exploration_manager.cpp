...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  // planner_manager_->path_finder_->max_search_time_ = 0.1;
  ...
  if (grid_ids.empty()) {
    ...
    if (ep_->refine_local_) {
      ...
      frontier_finder_->getViewpointsInfo(
          pos, { frontier_ids[0] }, ep_->top_view_num_, ep_->max_decay_, ed_->n_points_, n_yaws, ifsmallarea);
...
//7.28
  ...
  if(manyViewPoints){
    ...
    time_ratio = maxDiff / (maxDiff + temp_diff2);
  }

  planner_manager_->path_finder_->reset();
  planner_manager_->path_finder_->cur_vel = vel;
  ROS_ERROR("pos: %lf, %lf, %lf", pos[0], pos[1], pos[2]);
  ROS_ERROR("next pos: %lf, %lf, %lf", next_pos[0], next_pos[1], next_pos[2]);
...
int FastExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  if (len < radius_close || optimistic) {
    ...
    //   return FAIL;
    // }
    // vector<Vector3d> tmp_path = planner_manager_->path_finder_->getPath();
  ...
  t1 = ros::Time::now();
  double tmp_time = 0;
  planner_manager_->planYawExplore(yaw, next_yaw, true, ep_->relax_time_, tmp_time);
  double yaw_time = (ros::Time::now() - t1).toSec();
...
void FastExplorationManager::findGridAndFrontierPath(const Vector3d& cur_pos,
  ...
  vector<double> next_yaws = {cur_yaw[1]};
  // Partitioning-based tour planning
  vector<int> ego_ids;
  vector<vector<int>> other_ids;
  double next_yaw;
  Eigen::Vector3d start_yaw;
  if (!findGlobalTourOfGrid(positions, velocities, ego_ids, other_ids, next_yaws, {cur_yaw})) {
    grid_ids = {};
    return;
  }
  grid_ids = ego_ids;
...
void FastExplorationManager::shortenPath(vector<Vector3d>& path) {
  ...
  // hgrid_->activateGridsForShortcut(short_tour.front(), short_tour.back(), ep_->drone_id_);
...
void FastExplorationManager::refineLocalTour(const Vector3d& cur_pos, const Vector3d& cur_vel,
    ...
    vector<double>& refined_yaws, vector<bool> &smallareaData, vector<bool> &smallArea) {
...
void FastExplorationManager::allocateGrids(const vector<Eigen::Vector3d>& positions,
    ...
    vector<int>& other_ids, const vector<double>& next_yaws, const vector<Eigen::Vector3d>& start_yaws) {
  ...
  hgrid_->getCostMatrix(positions, velocities, first_ids, second_ids, grid_ids, mat, next_yaws, start_yaws);
...
double FastExplorationManager::computeGridPathCost(const Eigen::Vector3d& pos,
  ...
  double cost = 0.0;
  vector<Eigen::Vector3d> path;
  cost += hgrid_->getCostDroneToGrid(pos, grid_ids[0], first, velocities[0], next_yaw, start_yaw);
...
bool FastExplorationManager::findGlobalTourOfGrid(const vector<Eigen::Vector3d>& positions,
    ...
    const vector<double>& next_yaws, const vector<Eigen::Vector3d>& start_yaws, bool init) {
  ...
  hgrid_->updateGridData(
      ep_->drone_id_, grid_ids, ed_->reallocated_, ed_->last_grid_ids_, first_ids, second_ids, positions);
  ...
  if (!init)
    ...
    hgrid_->getCostMatrix(positions, velocities, { {} }, { {} }, grid_ids, mat, next_yaws, start_yaws);
